...
void GameInit()
{
	...
	for (int tri = 0; tri < 12; tri++)
	{
		...
		cube.plist[tri].attr = POLY_ATTR_SHADE_MODE_FLAT; //恒定着色
		cube.plist[tri].color = RGBABIT(0, 255, 0, 0);
	...
	diffuse.a = 0;
	specular.rgba = 0;
	int sun_light = Init_Light_LIGHT(0, LIGHT_STATE_ON, LIGHT_ATTR_INFINITE, ambient, diffuse, specular, NULL, &sun_dir, 0, 0, 0);
}
...
int main(int argc, char *argv[])
{
	...
	while (1)
	{
		...
		Transform_OBJECT(&cube, &mt, TRANSFORM_TRANS_ONLY);
		Remove_Backface_OBJECT(&cube, &camera);
		Light_Object_World(&cube, &camera, lights, MAX_LIGHTS);   //对物体进行光照计算
		
		Insert_OBJECT_RENDERLIST(&rend_list, &cube, false, true); //插入到渲染列表，使用light_color
